...
namespace LINQtoCSV
{
	internal class FieldMapper<T>
	{
		...
		public void WriteNames(List<string> row)
		{
			...
			for (var i = 0; i < m_IndexToInfo.Length; i++)
			{
		...
		public void WriteObject(T obj, List<string> row)
		{
			...
			for (var i = 0; i < m_IndexToInfo.Length; i++)
			{
				...
				{
					objValue = ((PropertyInfo)tfi.memberInfo).GetValue(obj, null);
				}
				else
				{
					objValue = ((FieldInfo)tfi.memberInfo).GetValue(obj);
				}
				...
				if (objValue != null)
				{
					...
					{
						resultString = ((IFormattable)objValue).ToString(tfi.outputFormat, m_fileDescription.FileCultureInfo);
					}
					else
					{
		...
		protected class TypeFieldInfo : IComparable<TypeFieldInfo>
		{
			...
			public int CompareTo(TypeFieldInfo other) => index.CompareTo(other.index);
