...
namespace LINQtoCSV
{
	...
	public class AggregatedException : LINQtoCSVException
	{
		...
		public AggregatedException(string typeName, string fileName, int maximumNbrExceptions) :
			base($"There were 1 or more exceptions while reading data using type \"{typeName}\". {FileNameMessage(fileName)}")
		{
	...
	{
		public BadStreamException() : base("Stream provided to Read is either null, or does not support Seek.")
		{
		}
	}
	...
	public class CsvColumnAttributeRequiredException : LINQtoCSVException
	{
		public CsvColumnAttributeRequiredException() :
			base("CsvFileDescription.EnforceCsvColumnAttribute is false, but needs to be true because " +
	...
	public class LINQtoCSVException : Exception
	{
		...
		public static string FileNameMessage(string fileName) => ((fileName == null) ? "" : " Reading file \"" + fileName + "\".");
	}
	...
	public class MissingCsvColumnAttributeException : LINQtoCSVException
	{
		public MissingCsvColumnAttributeException(string typeName, string fieldName, string fileName) :
			base($"Field \"{fieldName}\" in type \"{typeName}\" does not have the CsvColumn attribute. {FileNameMessage(fileName)}")
		{
			Data["TypeName"] = typeName;
			Data["FieldName"] = fieldName;
			Data["FileName"] = fileName;
	...
	public class MissingFieldIndexException : LINQtoCSVException
	{
		public MissingFieldIndexException(string typeName, int lineNbr, string fileName) :
			base($"Line {lineNbr} has more fields then there are fields or properties in type \"{typeName}\" with a FieldIndex. {FileNameMessage(fileName)}")
		{
			Data["TypeName"] = typeName;
	...
	public class MissingRequiredFieldException : LINQtoCSVException
	{
		public MissingRequiredFieldException(
			...
			int lineNbr,
			string fileName) :
			base($"In line {lineNbr}, no value provided for required field or property \"{fieldName}\" in type \"{typeName}\". {FileNameMessage(fileName)}")
		{
			Data["TypeName"] = typeName;
			Data["LineNbr"] = lineNbr;
			Data["FileName"] = fileName;
	...
	public class NameNotInTypeException : LINQtoCSVException
	{
		public NameNotInTypeException(string typeName, string fieldName, string fileName) :
			base($"The input file has column name \"{fieldName}\" in the first record, but there is no field or property with that name in type \"{typeName}\". {FileNameMessage(fileName)}")
		{
			Data["TypeName"] = typeName;
			Data["FieldName"] = fieldName;
			Data["FileName"] = fileName;
	...
	public class TooManyDataFieldsException : LINQtoCSVException
	{
		public TooManyDataFieldsException(string typeName, int lineNbr, string fileName) :
			base($"Line {lineNbr} has more fields then are available in type \"{typeName}\". {FileNameMessage(fileName)}")
		{
			Data["TypeName"] = typeName;
			Data["LineNbr"] = lineNbr;
			Data["FileName"] = fileName;
	...
	public class TooManyNonCsvColumnDataFieldsException : LINQtoCSVException
	{
		public TooManyNonCsvColumnDataFieldsException(string typeName, int lineNbr, string fileName) :
			base($"Line {FileNameMessage(fileName)} has more fields then there are fields or properties in type \"{lineNbr}\" with the CsvColumn attribute set. {typeName}")
		{
			Data["TypeName"] = typeName;
			Data["LineNbr"] = lineNbr;
			Data["FileName"] = fileName;
	...
	public class WrongDataFormatException : LINQtoCSVException
	{
		public WrongDataFormatException(
			...
			string fileName,
			Exception innerExc) :
			base($"Value \"{fieldValue}\" in line {lineNbr} has the wrong format for field or property \"{fieldName}\" in type \"{typeName}\". {FileNameMessage(fileName)}", innerExc)
		{
			Data["TypeName"] = typeName;
			Data["LineNbr"] = lineNbr;
			Data["FileName"] = fileName;
	...
	public class WrongFieldIndexException : LINQtoCSVException
	{
		public WrongFieldIndexException(string typeName, int lineNbr, string fileName) :
			base($"Line {lineNbr} has less fields then the FieldIndex value is indicating in type \"{typeName}\". {FileNameMessage(fileName)}")
		{
			Data["TypeName"] = typeName;
			Data["LineNbr"] = lineNbr;
			Data["FileName"] = fileName;
